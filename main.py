...
ax_kde.plot(z_vals, kde_real(z_vals), label="Real z", color="blue")
ax_kde.plot(z_vals, kde_fake(z_vals), label="GAN p_h(z)", color="orange", linestyle="--")
ax_kde.set_title(f"PDF Approximation from GAN Samples  (a_r={a_r}, b_r={b_r})")
ax_kde.set_xlabel("z")
ax_kde.set_ylabel("Density")
ax_kde.legend()
fig_kde.tight_layout()
fig_kde.savefig("pdf_kde_gan.png", dpi=150)
print("pdf_kde_gan.png saved")

fig_hist, ax_hist = plt.subplots(figsize=(8, 5))
ax_hist.hist(z, bins=60, density=True, histtype="step", linewidth=1.5, label="Real z", color="blue")
ax_hist.hist(z_fake, bins=60, density=True, histtype="step", linewidth=1.5, label="GAN z_f", color="orange")
ax_hist.set_title(f"Histogram: Real vs Generated  (a_r={a_r}, b_r={b_r})")
ax_hist.set_xlabel("z")
ax_hist.set_ylabel("Density")
ax_hist.legend()
fig_hist.tight_layout()
fig_hist.savefig("pdf_hist_gan.png", dpi=150)
print("pdf_hist_gan.png saved")

fig2, ax = plt.subplots(figsize=(7, 5))
ax.plot(g_loss_list, label="Generator", color="red")
ax.plot(d_loss_list, label="Discriminator", color="blue")
ax.set_title("GAN Training Behaviour")
